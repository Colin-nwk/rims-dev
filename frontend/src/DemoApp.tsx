...
export const useDemoTheme = () => {
    ...
    if (!context) throw new Error('useDemoTheme must be used within a ThemeProvider');
    return context;
};
...
export const useDemoToast = () => {
    ...
    if (!context) throw new Error('useDemoToast must be used within a ToastProvider');
    return context;
};
...
export const useDemoAuth = () => {
    ...
    if (!context) throw new Error('useDemoAuth must be used within an AuthProvider');
    return context;
};
...
export const useDemoNotifications = () => {
    ...
    if (!context) throw new Error('useDemoNotifications must be used within a NotificationProvider');
    return context;
};
...
export const useDemoDocuments = () => {
    ...
    if (!context) throw new Error('useDemoDocuments must be used within a DocumentProvider');
    return context;
};
...
export const useDemoPolicies = () => {
    const context = useContext(PolicyContext);
    if (!context) throw new Error('useDemoPolicies must be used within a PolicyProvider');
    return context;
};
...
export const useDemoApprovals = () => {
    const context = useContext(ApprovalContext);
    if (!context) throw new Error('useDemoApprovals must be used within an ApprovalProvider');
    return context;
};
...
export const useDemoComplaints = () => {
    const context = useContext(ComplaintContext);
    if (!context) throw new Error('useDemoComplaints must be used within a ComplaintProvider');
    return context;
};
...
const ProtectedRoute = ({ children, allowedRoles }: { children?: React.ReactNode; allowedRoles?: Role[] }) => {
    ...
    if (!user) return <Navigate to="/demo/login" state={{ from: location }} replace />;
...
const DemoAppContent = () => {
    ...
    const { approvals } = useDemoApprovals();
...
export default function DemoApp() {
    ...
    useEffect(() => {
        const storedUser = localStorage.getItem('nexus_demo_user');
        if (storedUser) {
    ...
    const login = (email: string, role: Role) => {
        ...
        setTimeout(() => {
            ...
            if (foundUser) {
                setUser(foundUser);
                localStorage.setItem('nexus_demo_user', JSON.stringify(foundUser));
