...
import { type ServiceType } from "./createService";
import {
  type ApiResponse,
...
export function createQueryHooks<
  ...
  CreateDTO = Partial<T>,
  UpdateDTO = Partial<T>,
>(service: ServiceType<T, CreateDTO, UpdateDTO>, queryKey: string) {
  return {
    /**
    ...
    useUpdate: (
      mutationOptions?: UseMutationOptions<
        ApiResponse<T>,
        ApiError,
        { id: string | number; data: UpdateDTO }
      >,
    ) => {
      const queryClient = useQueryClient();
    ...
    usePatch: (
      mutationOptions?: UseMutationOptions<
        ApiResponse<T>,
        ApiError,
        { id: string | number; data: Partial<UpdateDTO> }
      >,
    ) => {
      const queryClient = useQueryClient();
    ...
    useUpload: (
      mutationOptions?: UseMutationOptions<
        ApiResponse<T>,
        ApiError,
        { files: File | File[]; options?: UploadOptions }
      >,
    ) => {
      const queryClient = useQueryClient();
    ...
    useUploadForItem: (
      ...
      return useMutation<
        ...
        { id: string | number; files: File | File[]; options?: UploadOptions }
      >({
        mutationFn: ({ id, files, options }) =>
          service.uploadForItem(id, files, options),
        onSuccess: (_, variables) => {
          queryClient.invalidateQueries({ queryKey: [queryKey] });
          queryClient.invalidateQueries({
            queryKey: [queryKey, "detail", variables.id],
